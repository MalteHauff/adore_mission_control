...
MissionControl::update_route()
{
  ...
  {
    auto route = map::Route( ego, goals.front(), road_map );
  ...
  double remaining_distance = current_route->get_length() - current_route->get_s( ego );
  ...
  if( route_lane_id.has_value() )
  {
    ...
    for( size_t lane_id : corridor_lanes )
    {
      ...
      if( lit == road_map->lanes.end() || !lit->second )
      {
        continue;
      }

      if( lit->second->type != adore::map::LaneType::driving )
      {
        continue;
      }
    ...
    const bool cooldown_ok = ( now() - last_forced_replan_time_ ).seconds() > 0.5;
